...
# ネットワークグラフの作成
def visualize_graph(stations: pd.DataFrame, join: pd.DataFrame, distance_matrix: np.ndarray | None = None):
    G = nx.Graph()
    # 頂点を駅名にする
...
def calculate_distance_matrix(stations: pd.DataFrame, join: pd.DataFrame) -> np.ndarray:
    ...
    for station_cd1, station_cd2 in join[["station_cd1", "station_cd2"]].itertuples(index=False):
        if station_cd1 in station_index and station_cd2 in station_index:
            ...
            coord1 = stations[stations["station_cd"] == station_cd1][["lon", "lat"]].values[0]
            coord2 = stations[stations["station_cd"] == station_cd2][["lon", "lat"]].values[0]
...
def compute_key_station_pairwise_distances(all_stations: pd.DataFrame, all_join: pd.DataFrame, key_stations: pd.DataFrame) -> np.ndarray:
    """
...
def build_reduced_adj_matrix(all_stations: pd.DataFrame, all_join: pd.DataFrame, key_stations: pd.DataFrame) -> np.ndarray:
    """
...
def save_reduced_adjmatrix_csv(adj_matrix: np.ndarray, key_stations: pd.DataFrame, out_dir: Path, prefix: str = "key"):
    """
